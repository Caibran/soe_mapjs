...
export class Editor extends PhaserInstance {
  ...
  setupLightSourceEvents(scene) {
    scene.events.on("lightSourceUpdate", (entityState) => {
      this.dispatchEvent(
        new CustomEvent("light-source-update", { detail: entityState }),
      );
    });
  }
  ...
  onSceneReady(scene) {
    ...
    this.setupLightSourceEvents(scene);
