...
export class Application extends LitElement {
    ...
    if (this.validGfx() && this.mapState.loaded) {
      return html`
        <eomap-editor
          ...
          @light-source-update=${this.onLightSourceUpdate}
  ...
  async openFile(fileHandle) {
    ...
    try {
      ...
      // Load lighting JSON file if it exists
      await this.loadLightingFile(fileHandle);
    } catch (e) {
      let error = e;
  ...
  /**
   * Get the JSON lighting filename from the EMF filename
   */
  getLightingFilename(emfFilename) {
    return emfFilename.replace(/\.emf$/i, ".json");
  }

  /**
   * Load lighting data from JSON file alongside the EMF file
   */
  async loadLightingFile(emfFileHandle) {
    // Always start with defaults
    this.lightingState = LightingState.default();

    try {
      // For Electron, we can derive the path from the EMF file path
      if (emfFileHandle.path && window.bridge && window.bridge.fs) {
        const lightingPath = emfFileHandle.path.replace(/\.emf$/i, ".json");
        try {
          const response = await window.bridge.fs.readFile(lightingPath);

          // Bridge returns {error, returnValue} - extract the actual buffer
          if (response.error) {
            throw new Error(response.error);
          }
          const buffer = response.returnValue;

          // Buffer should be a Uint8Array at this point
          const blob = new Blob([buffer], { type: "application/json" });
          const text = await blob.text();
          const json = JSON.parse(text);
          this.lightingState = LightingState.fromJSON(json);
        } catch (readError) {
          // File doesn't exist or can't be read, use defaults (already set)
        }
      }
    } catch (e) {
      // lightingState already set to defaults above
    }
  }

  /**
   * Save lighting data to JSON file alongside the EMF file
   */
  async saveLightingFile() {
    if (!this.mapState.fileHandle || !this.lightingState) {
      return;
    }

    try {
      const lightingFilename = this.getLightingFilename(
        this.mapState.fileHandle.name,
      );
      const jsonString = JSON.stringify(this.lightingState.toJSON(), null, 2);
      const data = new TextEncoder().encode(jsonString);

      // For Electron, derive the path from the EMF file path
      if (this.mapState.fileHandle.path) {
        const lightingPath = this.mapState.fileHandle.path.replace(
          /\.emf$/i,
          ".json",
        );

        // Use the file system provider to write the file
        // We need to create a temporary file handle for the lighting file
        try {
          // Try to use the bridge if available (Electron)
          if (window.bridge && window.bridge.fs) {
            await window.bridge.fs.writeFile(lightingPath, data);
            return;
          }
        } catch (bridgeError) {
          console.warn("Failed to save lighting via bridge:", bridgeError);
        }
      }

      // For web, we would need a directory handle to create the file
      // This is a limitation of the File System Access API
      console.warn(
        "Lighting file saving not supported in web mode without directory access",
      );
    } catch (e) {
      console.error("Failed to save lighting file:", e);
    }
  }
  ...
  async save() {
    ...
    if (this.mapState.fileHandle === null) {
      ...
      try {
        ...
        // Save lighting file alongside EMF
        await this.saveLightingFile();
  ...
  showMapProperties() {
    this.properties.populate(this.mapState.emf, this.lightingState);
  ...
  onEntityEditorSave(event) {
    if (this.checkEntityOverflow(event.detail)) {
      ...
      // Update lighting state with new light sources
      if (event.detail.lightSources) {
        this.updateLightSourcesFromEntityState(event.detail);
      }
    } else {
      this.entityEditor.open = true;
    }
  ...
  updateLightSourcesFromEntityState(entityState) {
    if (!this.lightingState) {
      this.lightingState = LightingState.default();
    }

    const x = entityState.x;
    const y = entityState.y;

    // Remove existing light sources at this position
    const otherLightSources = this.lightingState.lightSources.filter(
      (ls) => ls.x !== x || ls.y !== y,
    );

    // Add the new light sources from the entity state
    const newLightSources = [...otherLightSources, ...entityState.lightSources];

    this.lightingState = this.lightingState.withLightSources(newLightSources);
  }

  onLightSourceUpdate(event) {
    // Handle light source updates from entity commands (e.g., Clear Entity, undo/redo)
    this.updateLightSourcesFromEntityState(event.detail);
  }
  ...
  onNewMapConfirm(event) {
    ...
    // Initialize lighting state with defaults for new maps
    this.lightingState = LightingState.default();
  }

  onPropertiesSave(event) {
    this.mapPropertiesState = event.detail.mapProperties;
    this.lightingState = event.detail.lightingState;
  }
