...
export class EOMap extends Phaser.GameObjects.GameObject {
  constructor(scene, textureCache, emf, layerVisibility) {
    ...
    // Lighting layer for rendering light effects
    this.lightingLayer = new LightingLayer(scene);
  ...
  setLightingState(lightingState) {
    this.lightingLayer.setLightingState(lightingState);
    this.invalidateCachedFrame();
  }
  ...
  getLightSourcesAt(x, y) {
    if (!this.lightingLayer.lightingState) {
      return [];
    }
    return this.lightingLayer.lightingState.getLightSourcesAt(x, y);
  }
  ...
  drawFrame() {
    ...
    // Apply lighting overlay directly to the render texture
    this.applyLightingToRenderTexture(renderTexture, drawWidth, drawHeight);
  }

  applyLightingToRenderTexture(renderTexture, width, height) {
    // Check if lighting should be applied
    if (
      !this.lightingEnabled ||
      !this.lightingLayer ||
      !this.lightingLayer.lightingState ||
      !this.lightingLayer.lightingState.globalLight ||
      this.lightingLayer.lightingState.globalLight.strength >= 10
    ) {
      return;
    }

    // Get the lighting canvas
    const lightCanvas = this.lightingLayer.renderToCanvas(
      width,
      height,
      this.scrollX,
      this.scrollY,
    ...
    const texKey = "__eomap_lighting_overlay__";

    // Create or get the persistent canvas for the lighting texture
    if (!this._lightingPersistentCanvas) {
      this._lightingPersistentCanvas = document.createElement("canvas");
    }

    // Check if we need to resize the persistent canvas
    const needsResize =
      this._lightingPersistentCanvas.width !== width ||
      this._lightingPersistentCanvas.height !== height;

    if (needsResize) {
      this._lightingPersistentCanvas.width = width;
      this._lightingPersistentCanvas.height = height;

      // Remove and recreate texture only when size changes
      if (this.scene.textures.exists(texKey)) {
        this.scene.textures.remove(texKey);
      }
    }

    // Copy the lighting canvas to the persistent canvas
    const ctx = this._lightingPersistentCanvas.getContext("2d");
    ctx.clearRect(0, 0, width, height);
    ctx.drawImage(lightCanvas, 0, 0);

    // Create texture if it doesn't exist
    if (!this.scene.textures.exists(texKey)) {
      this.scene.textures.addCanvas(texKey, this._lightingPersistentCanvas);
    }

    // Update the texture's WebGL texture from the canvas
    const tex = this.scene.textures.get(texKey);
    if (tex && tex.source && tex.source[0]) {
      // For WebGL, update the GL texture directly
      const glTexture = tex.source[0].glTexture;
      if (glTexture && this.scene.game.renderer.gl) {
        ...
        gl.texImage2D(
          ...
          gl.RGBA,
          gl.UNSIGNED_BYTE,
          this._lightingPersistentCanvas,
        );
      }
    }

    // Draw the lighting texture onto the render texture
    renderTexture.draw(texKey, 0, 0);
  }
  ...
  update(time, delta) {
    ...
    // Update lighting layer for flicker effects
    if (this.lightingLayer) {
      const wasDirty = this.lightingLayer.dirty;
      this.lightingLayer.update(time, delta);
      if (this.lightingLayer.dirty && !wasDirty) {
        this.invalidateCachedFrame();
      }
    }
  }
  ...
  destroy(fromScene) {
    ...
    if (this._lightingPersistentCanvas) {
      this._lightingPersistentCanvas = null;
    }
