...
import { isMac, isTouch } from "../util/platform-utils";
...
export class EditorScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.data.events.on("changedata-lightingState", () => {
  ...
  startPinchOrPan() {
    ...
    this.pinchStartZoom = this.map.zoom;

    this.panStartScrollX = this.map.camera.scrollX;
    this.panStartScrollY = this.map.camera.scrollY;

    this.prePanPointer1X = this.input.pointer1.x;
    this.prePanPointer1Y = this.input.pointer1.y;
    this.prePanPointer2X = this.input.pointer2.x;
    this.prePanPointer2Y = this.input.pointer2.y;

    // We'll decide whether it's a pinch or a pan in the first move
    this.isPinching = false;
    this.isTwoFingerPanning = false;
  }
  ...
  handlePinch() {
    ...
    if (!this.isPinching && !this.isTwoFingerPanning) {
      const distanceDelta = Math.abs(currentDistance - this.pinchStartDistance);
      if (distanceDelta > 10) {
        this.isPinching = true;
      } else {
        // Potentially pan if distance hasn't changed much but they moved together
  ...
  handleTwoFingerPan() {
    const midPointX = (this.input.pointer1.x + this.input.pointer2.x) / 2;
    const midPointY = (this.input.pointer1.y + this.input.pointer2.y) / 2;

    const prevMidPointX = (this.input.pointer1.prevPosition.x + this.input.pointer2.prevPosition.x) / 2;
    const prevMidPointY = (this.input.pointer1.prevPosition.y + this.input.pointer2.prevPosition.y) / 2;

    const dx = (prevMidPointX - midPointX) / this.map.zoom;
    const dy = (prevMidPointY - midPointY) / this.map.zoom;

    this.map.camera.scrollX += dx;
    this.map.camera.scrollY += dy;
  }

  startLongPressTimer(pointer) {
    this.cancelLongPressTimer();
  ...
  handleLongPress(pointer) {
    // Treat long press as a right-click
    ...
    this.tool.pointerDown(this, rightClickPointer);
    // Also trigger pointer up to finalize the "click" if it's a tool action
    setTimeout(() => {
      this.tool.pointerUp(this, rightClickPointer);
    }, 50);
  }
  ...
  updateEntityState(newEntityState) {
    ...
    // Get existing light sources at this position from lighting state
    let lightingState = this.data.get("lightingState");
    let oldLightSources = lightingState
      ? lightingState.lightSources.filter((ls) => ls.x === x && ls.y === y)
      : [];
    ...
    let oldEntityState = new EntityState(
      ...
      oldLightSources.map((ls) => ls.copy()),
    );

    // Create command with a callback to update lighting state
    let command = new EntityCommand(
      this.mapState,
      x,
      y,
      oldEntityState,
      newEntityState,
      (entityState) => this.dispatchLightSourceUpdate(entityState),
    );
    this.commandInvoker.add(command);
  }

  dispatchLightSourceUpdate(entityState) {
    // Dispatch event to update lighting state in the application
    this.events.emit("lightSourceUpdate", entityState);
  }
  ...
  updateLightingState() {
    let lightingState = this.data.get("lightingState");
    if (lightingState && this.map) {
      this.map.setLightingState(lightingState);
    }
  }
